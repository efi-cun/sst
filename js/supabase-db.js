...
const SupabaseDB = (() => {
    ...
    // Restablecer contraseña de un usuario en Supabase
    async function resetPassword(email, cedula, newPassword) {
        ...
        try {
            ...
            if (!user) {
                return { success: false, message: 'El correo y la cédula no coinciden con un usuario registrado' };
            }

            // Actualizar contraseña
            const { error: updateError } = await client
                .from('usuarios')
                .update({ password: btoa(newPassword) })
                .eq('cedula', cedula);
            ...
            console.error('Error en Supabase resetPassword:', e);
