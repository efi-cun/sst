...
const App = (() => {
    ...
    function setupEventListeners() {
        ...
        document.getElementById('show-recover').addEventListener('click', e => { e.preventDefault(); toggleAuth('recover'); });
        document.getElementById('show-login-from-recover').addEventListener('click', e => { e.preventDefault(); toggleAuth('login'); });
        document.getElementById('btn-recover').addEventListener('click', handleRecoverPassword);
        ...
        document.getElementById('recover-new-password').addEventListener('keydown', e => { if (e.key === 'Enter') handleRecoverPassword(); });
    ...
    async function handleRecoverPassword() {
        console.log('Recover password clicked. Reading values...');
        const emailEl = document.getElementById('recover-email');
        const idEl = document.getElementById('recover-id');
        const passEl = document.getElementById('recover-new-password');
        
        if (!emailEl || !idEl || !passEl) {
            console.error('Missing recover elements in DOM!');
            return;
        }
        ...
        const originalText = btnRecover.textContent;
        ...
        if (!email || !cedula || !newPassword) {
            showError('recover-error', 'Todos los campos son obligatorios');
            return;
        }
        ...
        try {
            ...
            const result = await AuthManager.resetPassword(email, cedula, newPassword);
            if (result.success) {
                if (successEl) {
                    successEl.textContent = '¡Contraseña restablecida con éxito! Redirigiendo al inicio de sesión...';
                ...
                showError('recover-error', result.message);
            }
        } catch (e) {
            console.error('Exception in handleRecoverPassword:', e);
            showError('recover-error', 'Error al restablecer la contraseña: ' + e.message);
        } finally {
            btnRecover.textContent = originalText;
    ...
    function toggleAuth(type) {
        ...
        if (recoverForm) recoverForm.classList.toggle('active', type === 'recover');
        hideError('login-error');
        hideError('register-error');
        hideError('recover-error');
        const recoverSuccess = document.getElementById('recover-success');
        if (recoverSuccess) recoverSuccess.classList.add('hidden');
    }
